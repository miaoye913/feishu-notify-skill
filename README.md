...
| 「我们聊会儿」「开启对话模式」 | **③ 对话模式** | `feishu_listen.py --listen` + `feishu_chat.py --wait-new` 循环 | ✅ 常驻（直到说「停」） |

判别口诀：**单向发 = 通知；发完要等你一句 = `--wait`；要来回多轮 = `--listen` + chat 循环**。

**状态不再模糊**：`feishu_chat.py --status` 随时可查监听是否在跑、有没有未读消息。

**消息区分**：发送时加 `--tag 通知` / `--tag 对话`，消息会带 `[通知]` / `[对话]` 前缀，一眼分清这条是通知还是对话。
