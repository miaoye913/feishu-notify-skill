...
## 0.5 三种模式怎么选（先看这张表，别搞混）

| 你的意图 | 模式 | 怎么做 | 需要监听 |
|---|---|---|---|
| 「任务跑完通知我」 | **① 通知模式** | `feishu_send.py "内容" --tag 通知` | ❌ 不需要 |
| 「问我一个问题，等我答」 | **② 单次问答** | `feishu_send.py` + `feishu_listen.py --wait` | ✅ 一次性（收到一条即退） |
| 「我们聊会儿」 | **③ 对话模式**（本文） | `feishu_listen.py --listen` + `feishu_chat.py --wait-new` 循环 | ✅ 常驻（直到说「停」） |

- 判别口诀：**单向发 = 通知；发完要等你一句 = `--wait`；要来回多轮 = `--listen` + chat 循环**
- 状态查询：`python feishu_chat.py --status` → 显示监听是否在运行（心跳）、未读数、模式提示
- 消息区分：发送加 `--tag 通知` / `--tag 对话`，消息带 `[通知]` / `[对话]` 前缀
- 只有 ③ 需要**常驻**监听；①② 用完即止，不存在"模式没退干净"的问题
